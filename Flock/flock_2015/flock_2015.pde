...
void setup() {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
            //sky.pixels[i + j * width] = color(255);
  ...
  flock.boids[30] = new Boid(  random(-100, 100),  random(-100, 100),  random(-100, 100), img);
...
void draw() {
  ...
  if (!isFirst) {
    ...
    third.perspective( radians(30), width/height, 1, 10000000);
    flock.display(third);
